...
```sh
bash <(curl -Ls https://raw.githubusercontent.com/jeqele/x-ui/master/install.sh)
```
...
```sh
VERSION=1.8.0 && bash <(curl -Ls "https://raw.githubusercontent.com/jeqele/x-ui/$VERSION/install.sh") $VERSION
```
...
| `POST` | `"/importbackup"`                  | Import a database backup file             |
